...
namespace Colossal.Mods
{
    public class Platforms : MonoBehaviour
    {
        ...
        public void Update()
        {
            ...
            {
                bool leftGrab = UniversalInputManager.Grabbing(XRNode.LeftHand);
                bool rightGrab = UniversalInputManager.Grabbing(XRNode.RightHand);
                if (leftGrab)
                {
                    if (!this.PlatLonce)
                    {
            ...
            {
                Destroy(holder.GetComponent<Platforms>());
            }
        }
    }
}
