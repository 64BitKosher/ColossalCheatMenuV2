...
namespace Colossal.Mods
{
    public class SpeedMod : MonoBehaviour
    {
        public void Update()
        {
                ...
                if (UniversalInputManager.Grabbing(XRNode.LeftHand))
                {
                ...
                else if (!UniversalInputManager.Grabbing(XRNode.RightHand) && Menu.Menu.Speed[1].stringsliderind != 0)
                    if (GorillaTagger.Instance.offlineVRRig.mainSkin.material.name.Contains("fected"))
                        GorillaLocomotion.Player.Instance.maxJumpSpeed = 8.5f;
                    else
                        GorillaLocomotion.Player.Instance.maxJumpSpeed = 6.5f;

                if (UniversalInputManager.Grabbing(XRNode.RightHand))
                {
                ...
                else if (!UniversalInputManager.Grabbing(XRNode.LeftHand) && Menu.Menu.Speed[2].stringsliderind != 0)
                    if (GorillaTagger.Instance.offlineVRRig.mainSkin.material.name.Contains("fected"))
                        GorillaLocomotion.Player.Instance.maxJumpSpeed = 8.5f;
                    else
                        GorillaLocomotion.Player.Instance.maxJumpSpeed = 6.5f;
